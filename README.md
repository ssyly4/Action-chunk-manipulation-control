...
```bash
./scripts/run_policy_native.sh --preflight-only
./scripts/run_policy_native.sh --execute
```
...
scripts/run_policy_osqp_casadi.sh
  └─ trajectory/osqp_waypoint_smoother/ab_runtime/run_30k_osqp_casadi.sh
       └─ trajectory/osqp_waypoint_smoother/ab_runtime/run_policy_trial_osqp_casadi.sh
            ├─ nero_neo_teleop/scripts/can/ensure_can_interface.sh
            ├─ scripts/bimanual_policy/ensure_bimanual_policy_server.sh
            └─ bimanual_guarded_policy_stream_osqp_casadi.py
                 └─ 进程内加载 bimanual_guarded_policy_stream.py
                    并替换 RTC queue 与 follower
```

这样做的目的，是让原生策略流保持可对照，同时只在 A/B 进程内接入 OSQP、CasADi、TOPPRA 相关逻辑。
...
| 文件 | 用途 |
|---|---|
| `scripts/run_policy_osqp_casadi.sh` | 当前默认入口；读取本机路径配置后启动 OSQP + CasADi 实机 A/B。 |
| `scripts/run_policy_native.sh` | 原生 RTC 对照入口，不注入轨迹优化器。 |
| `config/paths.env.example` | 本机目录、SDK、策略服务地址模板。复制后形成不入 Git 的 `config/paths.env`。 |
| `docs/CURRENT_CONTROL_STACK.md` | 当前控制参数、运行规则与验证命令。 |

### 原生双臂策略运行器：`scripts/bimanual_policy/`
...
| `run_bimanual_policy_trial.sh` | 原生实机启动器：CAN 预检、策略预热、可选回零、执行与失败后的 hold。 |
...
| `osqp_waypoint_smoother/ab_runtime/osqp_casadi_rtc_queue.py` | 当前主 A/B queue：串接 OSQP、CasADi、TOPPRA 交接，并处理恢复候选。 |
| `osqp_waypoint_smoother/ab_runtime/bimanual_guarded_policy_stream_osqp_casadi.py` | 运行时注入适配器：动态加载原生策略流，并替换 queue/follower 类。 |
| `osqp_waypoint_smoother/ab_runtime/run_policy_trial_osqp_casadi.sh` | A/B 实机编排器：CAN 准备、策略服务、可选回零和 Python 环境。 |
| `osqp_waypoint_smoother/ab_runtime/run_30k_osqp_casadi.sh` | 当前 30k 模型与 OSQP/CasADi 参数 preset；通过环境变量暴露调参面。 |
| `casadi_fixed_horizon_retimer/fixed_phase_optimizer/optimizer.py` | 固定总时长的相位优化器，支持速度、加速度、jerk 约束和 warm-start。 |
| `casadi_fixed_horizon_retimer/ab_runtime/casadi_rtc_queue.py` | 将 CasADi 相位优化接入滚动 RTC queue 的适配层。 |
| `toppra_fixed_horizon_retimer/fixed_path_retimer/` | 路径表示、TOPPRA 重定时与滚动计划数据结构。 |
| `toppra_fixed_horizon_retimer/ab_runtime/receding_toppra_queue.py` | q/v/a 连续 handoff、reserve、commit、候选拒绝与重新请求策略。 |
| `toppra_fixed_horizon_retimer/ab_runtime/follower_state_bridge.py` | 在 queue 与 follower 之间共享最后发送命令、速度、加速度和期望速度。 |
...
推荐只通过环境变量调当前 OSQP + CasADi A/B，而不要直接改 Python 默认值：
