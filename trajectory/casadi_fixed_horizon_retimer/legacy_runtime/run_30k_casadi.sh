...
export NERO_POLICY_STAGE_NAME=nero_towel_fullflow70_releasecrop_tailpush30_h24_30000_casadi_ab
...
export NERO_TOPPRA_RUNTIME_LOG="${NERO_CASADI_RUNTIME_LOG:-$ROOT/../outputs/runtime_$(date +%Y%m%d_%H%M%S).jsonl}"
...
export NERO_TOPPRA_MIN_COMMIT_TICKS="${NERO_TOPPRA_MIN_COMMIT_TICKS:-12}"
...
exec "$ROOT/run_policy_trial_casadi.sh" \
  ...
  --max-command-error-deg 2.25 --feedback-governor-error-deg 1.25 \
