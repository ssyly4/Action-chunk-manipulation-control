...
| `trajectory/casadi_fixed_horizon_retimer/legacy_runtime/` | 已替代的 A/B 启动器 | 直接 CasADi 实机运行器；优化器本身仍在当前链路中使用。 |
| `trajectory/toppra_fixed_horizon_retimer/legacy_runtime/` | 已替代的 A/B 启动器 | 直接 TOPPRA 实机运行器；共享队列与桥接层仍是当前依赖。 |
