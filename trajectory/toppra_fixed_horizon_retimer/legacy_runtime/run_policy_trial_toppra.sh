...
if [[ "$execute" == 0 ]]; then
  exec "$PYTHON" "$ROOT/bimanual_guarded_policy_stream_toppra.py" \
    "${common[@]}" --preflight-only "$@"
fi

if [[ "${NERO_POLICY_SKIP_HOME:-0}" != 1 ]]; then
  echo "[TRIAL] returning both arms to demonstration Home before TOPPRA A/B"
  "$TELEOP_ROOT/scripts/control/run_dual_home.sh" --execute
fi

set +e
"$PYTHON" "$ROOT/bimanual_guarded_policy_stream_toppra.py" \
  "${common[@]}" \
...
if [[ "$status" != 0 ]]; then
  echo "[TRIAL] TOPPRA A/B failed (exit=$status); automatic Home is disabled" >&2
  exit "$status"
fi

if [[ "${NERO_POLICY_SKIP_HOME:-0}" != 1 ]]; then
  echo "[TRIAL] TOPPRA A/B succeeded; returning both arms Home"
  "$TELEOP_ROOT/scripts/control/run_dual_home.sh" --execute
fi
